...
docker_prod_or_debug: ## [INTERACTIF] Choisir le mode production (sécurisé) ou degub (non sécurisé) NB: .env configuré
	...
	@echo "PATH=$$PATH" >> .env
	@echo "PATH=$$PATH" >> .env.config
	@echo "USER_ID=$$USER_ID" >> .env
	@echo "USER_ID=$$USER_ID" >> .env.config
	@echo "GROUP_ID=$$GROUP_ID" >> .env
	@echo "GROUP_ID=$$GROUP_ID" >> .env.config
	@echo "AIRFLOW_UID=$$AIRFLOW_UID" >> .env
	@echo "AIRFLOW_UID=$$AIRFLOW_UID" >> .env.config
	@echo "AIRFLOW_GID=$$AIRFLOW_GID" >> .env
	@echo "AIRFLOW_GID=$$AIRFLOW_GID" >> .env.config
	@echo "MLFLOW_TRACKING_URI=$$MLFLOW_TRACKING_URI" >> .env
	@echo "MLFLOW_TRACKING_URI=$$MLFLOW_TRACKING_URI" >> .env.config
	@echo "DOCKER_MLFLOW_TRACKING_URI=$$DOCKER_MLFLOW_TRACKING_URI" >> .env
	@echo "DOCKER_MLFLOW_TRACKING_URI=$$DOCKER_MLFLOW_TRACKING_URI" >> k8s/airflow/.env.config
	@echo "EVIDENTLY_PATH=$$EVIDENTLY_PATH" >> .env
	@echo "EVIDENTLY_PATH=$$EVIDENTLY_PATH" >> .env.config
	@echo "WORKSPACE_NAME=$$WORKSPACE_NAME" >> .env
	@echo "WORKSPACE_NAME=$$WORKSPACE_NAME" >> .env.config
	@echo "EVIDENTLY_WORKSPACE=$$EVIDENTLY_WORKSPACE" >> .env
	@echo "EVIDENTLY_WORKSPACE=$$EVIDENTLY_WORKSPACE" >> .env.config
