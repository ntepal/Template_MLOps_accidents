...
install: ## [INIT] Installation/Initialisation complète du projet
	...
	@#uv pip install -e .
	@mkdir -p logs
	@echo "📦 Installation du projet en mode éditable (logs à la racine dans logs/pip_intall-e.log.)..."
	@uv pip install -e . > logs/pip_intall-e.log 2>&1 || { \
		echo "❌ Erreur : Installation éditable échouée ! Voir logs/pip_intall-e.log pour les détails."; \
		exit 1; \
	}
	@echo "✅ Installation (pip install -e) éditable réussie."
	...
	@echo "⏳ Cela prend plusieurs secondes. Affichage masqué pour plus de clarté"
	@#uv sync || (echo "❌ Erreur 1 : pyproject.toml introuvable ou corrompu !"; exit 1)
	@echo "✅ uv synchronisation réussie."
	...
	@if [ -f ".AccidentsSetupDVC_AlreadyDone" ]; then \
		...
		echo "Fichiers et répertoires supprimés"; \
		uv run dvc init --no-scm; \
		echo "dvc init fait avec succès"; \
		mkdir -p .dvc/cache; \
		echo ".dvc/cache créé avec succès"; \
		sudo chown -R $(USER):$(USER) .dvc; \
		echo ".dvc avec les droits users"; \
	...
	@grep -qxF "/.dvc/config" .gitignore || echo "/.dvc/config" >> .gitignore
...
docker-reset-for-full-simu: ## [PROD][DOCKER] Reset DVC (lock/cache) et data (raw/preprocessed) pour simu propre
	...
	@sudo rm -rf dvc.lock .dvc/cache/* .dvc/tmp/* data/raw/* data/preprocessed/*
...
# Lancer tout l'écosystème conteneurisé
...
docker-full-start-WoInitialTrain_fast: ## [PROD][DOCKER] Démarrage simultannés des services Postgres et Redis, puis Airflow-Init et enfin tous les autres sans Initial Train
	...
	@echo "🌬️ Lancement de l'initialisation de Airflow (airflow-init)..."
...
kubernetes-deploy-service:
	...
	@#kubectl apply -k k8s/base/$(SERVICE)/ -n accidents-severity
	@# Pour mémoire certains services vont chercher les fichiers dans d'autres répertoires et pour éviter les refus pour cause de sécurité on rajoute le load restrictor
	@kubectl kustomize k8s/base/$(SERVICE)/ --load-restrictor LoadRestrictionsNone | kubectl apply -f - -n accidents-severity

	@#echo " ⏳ ATTENTE DU ROLLOUT (timeout=$(TIMEOUT))..."
	@# rollout status renvoie un code erreur (1) si le déploiement échoue
	@#kubectl rollout status deployment/$(SERVICE) -n accidents-severity --timeout=$(TIMEOUT)

	@# WARNING: Test si c'est un statefulset, sinon on part du principe que c'est un deployment
...
kubernetes-deploy-job:
	@echo " 🚀 LANCEMENT JOB: $(JOB) depuis $(DIR)"
	@#kubectl apply -k k8s/base/jobs/$(DIR) -n accidents-severity
	@# Pour mémoire le airflow init et set vont chercher les fichiers config et secret dans airflow et refus pour cause de sécurité
	@# On ajoute --load-restrictor none pour permettre de lire les fichiers en dehors du dossier
	@kubectl kustomize k8s/base/jobs/$(DIR) --load-restrictor LoadRestrictionsNone | kubectl apply -f - -n accidents-severity

	@echo " ⏳ ATTENTE COMPLETION (timeout=$(TIMEOUT))..."
	@# On enchaîne tout dans le même bloc shell pour conserver la valeur de $?
	@kubectl wait --for=condition=complete --timeout=$(TIMEOUT) job/$(JOB) -n accidents-severity; \
	RET_CODE=$$?; \
...
kubernetes-start: ## [PROD][KUBERNETES] Démarrage simultanés des services Postgres et Redis, puis Airflow-Init et enfin tous les autres sans Initial Train
	...
	@echo "🚀 Déploiement..."
	@# kubectl apply -k k8s/
	@echo "📍📍📍 ---- LANCER TOUS LES SERVICES SIMULTANNEMENT RALENTIT LA VM (CPU=100%) PAR MANQUE DE RAM ---"
	...
	@#kubernetes-deploy-job airflow-set-variables 600s
	@#echo "======================================================================"
	...
	@# ===== FIN COMMENTAIRE - TOUT EST MIS EN COMMENTAIRE CAR DEJA FAIT JUSTE AVANT ====
	...
	@echo "Vérification des volumes après le full restart"
	@#docker volume ls
	@kubectl get pvc -n accidents-severity
...
drift-on: ## [PROD][DOCKER] Modifier le fastapi_data.csv pour activer le drift dans EVIDENTLY et générer l'alarme dans GRAFANA
	...
	@echo "Copier dans fastapi_data.csv le fichier (à partir de son répertoire) qui génère le drift"
	@sudo cp simu_data_drift/fastapi_data_ForDriftAlerterDemo.csv data/users/fastapi_data.csv
	@echo "⏳ et Vérifier dans Evidently 'Drift is detected' et dans Grafana l'alarme active"
	@echo "Vérifier que le message est reçu sur https://webhook.site/id (id défini dans grafana Alerting/Contact points)"

drift-off: ## [PROD][DOCKER] Revenir au fastapi_data.csv normal pour désactiver le drift dans EVIDENTLY (alarme dans GRAFANA redevient normale)
	@echo "On copie fastapi_data_backup.csv par fastapi_data.csv pour revenir au fichier d'origine"
	@# Ne pas faire le mv car dans ce cas, c'est la date originale du bakcup qui est prise
	@# et comme elle est ancienne, elle ne déclenche pas une nouvelle analyse
	@# alors que le cp fait comme si on venait d'updater le fichier
	@sudo cp data/users/fastapi_data_backup.csv data/users/fastapi_data.csv
	@sudo rm -f data/users/fastapi_data_backup.csv
	@echo "⏳ et Vérifier dans Evidently 'Drift is NOT detected' et dans Grafana l'alarme désactivée"
	@echo "Vérifier qu'aucun message est reçu sur https://webhook.site/id (id défini dans grafana Alerting/Contact points)"
...
kubernetes-ubuntu-usage: ## [PROD][DOCKER] UBUNTU : VERIF RAM, DISK, CPU
	@echo "--- RAM ---"
	@free -h
	@echo ""
	@echo "--- DISK ---"
...
docker-shell-mlflow: ## [DEBUG] Entrer dans le serveur MLFlOW (contenu des répertoires)
	...
	@echo " ==========================================================================="
	docker exec -it -u root mlflow_server sh
...
docker-shell-postgres: ## [DEBUG] Entrer dans la base de donnée POSTGRES
	...
	@echo " ------ Pour sortir du shell, taper la commande: exit"
	@echo " ==========================================================================="
	docker exec -it -u root postgres_db sh

db-psql-postgres-data: ## [DEBUG] Vérifier si les runs sont bien enregistrés
	@echo "--- Liste des tables MLflow dans POSTGRES ---"
	docker exec -it -u root postgres_db psql -U airflow -d airflow -c "\dt"

db-psql-postgres-disk: ## [DEBUG] Voir la taille réelle occupée sur le disque
	docker exec -it -u root postgres_db du -sh /var/lib/postgresql/data

variables: ## [DEBUG] Voir si les variables dans docker-compose.yml sont bien comprises
